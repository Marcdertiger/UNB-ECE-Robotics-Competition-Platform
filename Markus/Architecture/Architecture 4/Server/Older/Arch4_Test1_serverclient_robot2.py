...
db = MySQLdb.connect("localhost", "robot2", "therobot2", "Controller",cursorclass = MySQLdb.cursors.SSCursor)
curs=db.cursor()

insertquery = ("""INSERT INTO status(name,cdate,ctime,enmotors,pwml,pwmr,pwma1,pwma2,report) Values(%s, %s, %s, %s, %s, %s, %s, %s, %s)""")

deletequery = ("DELETE FROM status WHERE name = 'robot2' ")
...
print "Collecting updates from Robot 2..."
socketS.connect ("tcp://131.202.14.123:%s" % port)    #Robot ip address

if len(sys.argv) > 2:
    socketS.connect ("tcp://131.202.14.123:%s" % port1)
